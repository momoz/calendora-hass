...
**Source:** `momoz/calendora` @ `d530bc1`
**Contract fingerprint:** `fbb6fb7286600856` — see §10
...
**Branch on `code`, never on the sentence.** Sentences change; codes do not.
...
| Somebody else changed the event between your request arriving and it being written | 400 saying so. **Retry is safe** — the second attempt reads the row as it now stands |

That last one matters for a multi-row change. Rather than half-apply a detach —
which would show the day twice — the server refuses the whole thing and tells
you.
...
`fixtures/api-v1/` in the Calendora repository — **34 request/response pairs**,
...
`write-occurrence-following`, `error-scope-missing` and
`error-occurrence-not-on-that-day`.
